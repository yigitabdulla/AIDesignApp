...
import ImageItem from "../view/object/image"
import useSelection from "../hook/useSelection"
import useTab from "../hook/useTab"
import useTransformer from "../hook/useTransformer"
import useStage from "../hook/useStage"
import useTool from "../hook/useTool"
import TextItem from "../view/object/text"
import ShapeItem from "../view/object/shape"
import IconItem from "../view/object/icon"
import LineItem from "../view/object/line"
...
function App() {
  ...
  const createStageDataObject = item => {
    const { id } = item.attrs
    const target =
  ...
  const renderObject = item => {
    switch (item.attrs["data-item-type"]) {
      case "frame":
        return (
          <Frame key={`frame-${item.id}`} data={item} onSelect={onSelectItem} />
        )
      case "image":
        return (
          <ImageItem
  ...
  useEffect(() => {
    ...
    onCreateTab(undefined, initialStageDataList[0])
